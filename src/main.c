...
void testar_algoritmo(const char* nome_algoritmo, int sizes[], int num_sizes, 
    ...
    for (int exec = 0; exec < REPETICOES; exec++) {
        ...
        for (int i = 0; i < num_sizes; i++) {
            ...
            printf("| %7d | %14.2f | %15.2f | %13.2f |\n", 
                ...
                soma_cresc[i] += temp_cresc[i];
                soma_dec[i] += temp_dec[i];
                soma_rand[i] +=  temp_rand[i];
        ...
        printf("media dos tempos de execução (ms):\n");
        printf("-------------------------------------------------------------\n");
        for (int i = 0; i < num_sizes; i++) {
            printf("| %7d | %14.2f | %15.2f | %13.2f |\n", 
    ...
    //star_algoritmo("Cocktail Shaker Sort", sizes, num_sizes, cocktail_shaker_sort);
